...
* **`tests/test_api_endpoints.py`**：檢驗 `/health`、`/api/oils`、`/api/indicators`、`/api/draws/health`、`/api/draws`、`/api/draws/redeem`、`/api/log-draw` 狀態碼與 JSON 結構，以及反向代理多重 IP 辨識安全。
...
* **`tests/test_js_syntax.js`**：以 Node.js 執行全站 13 個 JavaScript 模組的靜態編譯與 AST 語法安全性檢測。

### 4. L6 自動化調度與 CI/CD 管線
* **`tests/run_all_tests.py`**：Agent 3 專屬一鍵執行全量測試調度器，自動偵測 Python 與 Node.js 環境，輸出結構化報表與耗時分析。
