# 🧪 Agent 1：自動化測試與品質檢驗深度架構 (`/tests/`)

本目錄為 **Agent 1（品管與測試工程師專屬）** 唯一管轄維護範圍，掌管全站自動化測試矩陣、API 整合校驗、單元測試、E2E 結帳整合、日式美學規範稽核、正式端點活體探測與 CI/CD 自動化建置。
...
```
┌───────────────────────────────────────────────────────────────────────────────┐
│ Agent 1 自動化測試深度階梯架構（Depth Pyramid 2.0）                           │
├───────────────────────────────────────────────────────────────────────────────┤
...
### 1. L3 & L5 後端 API、第三方金流與安全防禦 (針對 Agent 2 / Agent 5)
...
* **`tests/test_security_audit.py`**：**[全新照妖鏡防線]** 檢驗全站 Stored/DOM XSS 實體轉義（`static/inventory.js`、`components/cart/cart.js`、`static/booking.js`、`static/admin.js`、`static/sites.js`）、`core/keep_warm.py` 外部 Ingress 保溫探針、`core/payment_manager.py` 訂單 Supabase PostgreSQL 雙寫持久化與冷啟動檢索恢復，以及衛福部自然醫學零違規詞彙強制斷言。
...
### 3. L1 & L4 前端元件、購物車與日式款待美學 (針對 Agent 3)
...
供 **Agent 2（後端）**、**Agent 3（前端）**、**Agent 4（文案）**、**Agent 5（維運）** 隨時查閱並作為下一輪迭代之具體優化指引。