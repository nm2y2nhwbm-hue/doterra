(function(){
  ...
  const cardDataState = document.getElementById('card-data-state');
  const cardDataRetry = document.getElementById('card-data-retry');
  ...
  async function loadData(){
    ...
    try {
      const [oilsRes, indRes] = await Promise.all([
        fetch(`${API_BASE}/api/oils`, { signal: controller.signal }),
        fetch(`${API_BASE}/api/indicators`, { signal: controller.signal })
      ]);
      if (!oilsRes.ok || !indRes.ok) throw new Error('牌卡服務回應異常');
  ...
  function setDataControlsEnabled(enabled){
    dataReady = enabled;
    categoryRow.querySelectorAll('.category-card').forEach((button) => { button.disabled = !enabled; });
    cdModes.querySelectorAll('.mode-btn').forEach((button) => { button.disabled = !enabled; });
  }
  ...
  function renderCategoryDetail(catKey){
    ...
    categoryRow.querySelectorAll('.category-card').forEach(el => {
      const isActive = el.dataset.cat === catKey;
    ...
    cdModes.innerHTML = cat.modes.map(m => {
      ...
      return `<button type="button" class="mode-btn" data-mode="${m}"${dataReady ? '' : ' disabled'}><span class="mode-num">${num}</span><span class="mode-text"><b>${cfg.title}</b><span>${cfg.desc || ''}</span></span></button>`;
    }).join('');
  }
  ...
  function ensureInsightModal(){
    ...
    el.setAttribute('aria-labelledby', 'im-name');
    ...
    el.addEventListener('keydown', (event) => {
      ...
      const focusable = Array.from(el.querySelectorAll('a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])'));
  ...
  function openInsightModal(label, card){
    ...
    insightPreviousFocus = document.activeElement;
    ...
    window.requestAnimationFrame(() => el.querySelector('.insight-modal-close').focus());
  }

  function closeInsightModal(){
    if (!insightModalEl || !insightModalEl.classList.contains('open')) return;
    insightModalEl.classList.remove('open');
    insightModalEl.setAttribute('aria-hidden', 'true');
    if (insightPreviousFocus && document.contains(insightPreviousFocus)) insightPreviousFocus.focus();
    insightPreviousFocus = null;
  }
  ...
  if (window.ModernOilGuide) {
    window.ModernOilGuide.setup('guide-book-link');
  }
  ...
  function initFromQuery(){
    ...
    if (params.get('guide') === '1' && window.ModernOilGuide) window.ModernOilGuide.open();
  ...
  async function startDataFlow(){
    setDataControlsEnabled(false);
    cardDataState.hidden = false;
    cardDataState.textContent = '正在載入牌卡資料…';
    cardDataRetry.hidden = true;
    try {
      await Promise.all([loadData(), liffInitialization]);
      setDataControlsEnabled(true);
      cardDataState.textContent = '牌卡資料已準備完成';
      window.setTimeout(() => { cardDataState.hidden = true; }, 1200);
      instruction.textContent = '';
      ...
      setDataControlsEnabled(false);
      cardDataState.hidden = false;
      cardDataState.textContent = e && e.name === 'AbortError'
        ? '牌卡載入逾時，請重新嘗試。'
        : '牌卡資料讀取失敗，請重新嘗試。';
      cardDataRetry.hidden = false;
    }
  }

  cardDataRetry.addEventListener('click', startDataFlow);
  startDataFlow();
})();
